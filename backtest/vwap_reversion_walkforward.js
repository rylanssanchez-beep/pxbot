...
// Same walk-forward discipline as orb_walkforward.js/walkforward.js: expanding
// train window, grid-search on train only, score the picked config on unseen
// test data. Applied to the new VWAP mean-reversion strategy.

const http = require('http');
const { runVwapReversionBacktest } = require('./vwap_reversion_engine');
...
function buildGrid() {
  const grid = [];
  for (const deviationAtrMultiple of [1.5, 2, 2.5, 3]) {
    for (const targetVwapFraction of [0.3, 0.5, 0.75, 1.0]) {
      for (const slBufferPct of [0.05, 0.1, 0.2]) {
        for (const maxTrendEfficiency of [0.3, 0.4, 0.5, 1.0]) { // 1.0 = effectively no trend filter, for comparison
          for (const [dayFilterName, allowedDaysOfWeek] of Object.entries(DAY_FILTERS)) {
            grid.push({ deviationAtrMultiple, targetVwapFraction, slBufferPct, maxTrendEfficiency, allowedDaysOfWeek, dayFilterName });
          }
        }
      }
    }
...
(async () => {
  ...
  const naive = runVwapReversionBacktest(bars, {});
  ...
  for (let f = 1; f < NUM_FOLDS; f++) {
    ...
    const scored = grid.map(params => {
      const r = runVwapReversionBacktest(train, params);
    ...
    const testR = runVwapReversionBacktest(test, best.params);
    const result = {
    ...
    console.log(`Fold ${f} [${result.testRange}] picked ${JSON.stringify({ dev: best.params.deviationAtrMultiple, tgt: best.params.targetVwapFraction, slBuf: best.params.slBufferPct, maxER: best.params.maxTrendEfficiency, days: best.params.dayFilterName })} (train avgR ${result.trainAvgR})`);
  ...
  console.log(`\n=== VWAP REVERSION WALK-FORWARD VERDICT ===`);
  ...
  fs.writeFileSync(path.join(__dirname, 'vwap_reversion_results.json'), JSON.stringify({ naive, foldResults }, null, 2));
  console.log('\nWritten to backtest/vwap_reversion_results.json');
